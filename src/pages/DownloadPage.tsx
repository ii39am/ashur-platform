...
export function DownloadPage() {
  ...
  return <main className="min-h-screen bg-dark-950 px-4 pb-20 pt-32">
    <div className="mx-auto max-w-6xl">
      ...
      {builds.length === 0 ? <section className="mt-10 rounded-3xl border border-white/10 bg-dark-800 p-8"><h2 className="text-2xl font-bold text-white">Builds are temporarily unavailable</h2><p className="mt-3 max-w-2xl text-slate-300">No installer with complete, verified metadata and a real download URL is configured. Nothing will download automatically.</p><Link to="/support" className="mt-6 inline-flex min-h-11 items-center text-brand-400">Support availability</Link></section> : <div className="mt-10 grid gap-5 md:grid-cols-2">{builds.map((build) => <article key={build.id} className="rounded-3xl border border-white/10 bg-dark-800 p-7"><h2 className="text-2xl font-bold text-white">{build.productName}</h2><p className="mt-2 text-slate-300">Version {build.version} · {build.architecture}</p><a href={build.publicDownloadUrl} className="auth-submit mt-6" download>Download for {build.platform}</a></article>)}</div>}
