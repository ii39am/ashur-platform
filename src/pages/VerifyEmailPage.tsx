...
import { isValidSignupEmailOtp, OtpInput } from '../components/auth/OtpInput';
...
export function VerifyEmailPage() {
  ...
  const validCode = isValidSignupEmailOtp(code);
  const verify = async (event: React.FormEvent) => { event.preventDefault(); if (operationLock.current) return; if (!navigator.onLine) { setError(copy.common.offline); return; } if (attempts >= onboardingConfiguration.maxOtpAttemptsPerPageSession) { setError(copy.verify.attempts); return; } if (!validCode || !email) { setError(email ? copy.verify.invalid : copy.verify.missing); return; } operationLock.current = true; setSubmitting(true); setError(''); try { const result = await authService.verifySignupOtp(email, code); if (!result.session) throw new Error('invalid otp'); auth.adoptSession(result.session); await finishProfile(); } catch (reason) { setAttempts((value) => value + 1); setError(otpError(reason, copy.verify)); setCode(''); } finally { operationLock.current = false; setSubmitting(false); } };
  ...
  return <AuthShell title={copy.verify.title} description={copy.verify.description}><OnboardingProgress labels={copy.steps} current={2} /><p className="mt-4 text-center text-sm text-slate-300" dir="ltr">{maskEmail(email)}</p>
    {error && <FormFeedback type="error">{error}</FormFeedback>}{success && <FormFeedback type="success">{success}</FormFeedback>}
    {setup ? <div className="py-6 text-center text-slate-300" aria-busy="true">{copy.verify.setup}</div> : auth.verified && !auth.profile ? pending?.payload ? <button type="button" onClick={retrySetup} disabled={submitting} className="auth-submit mt-5">{copy.verify.retrySetup}</button> : <Link to="/create-account" state={{ from: destination }} className="auth-submit mt-5">{copy.verify.retrySetup}</Link> : <form onSubmit={verify} className="mt-6 space-y-5"><OtpInput value={code} onChange={setCode} disabled={submitting} digitLabel={copy.verify.digit} /><button disabled={submitting || !validCode || attempts >= onboardingConfiguration.maxOtpAttemptsPerPageSession} className="auth-submit">{submitting ? copy.verify.verifying : copy.verify.verify}</button></form>}
