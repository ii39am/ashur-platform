...
export function PricingPage() {
  ...
  return <main className="min-h-screen bg-dark-950 px-4 pb-20 pt-32">
    <div className="mx-auto max-w-6xl">
      ...
      {confirmedPricingPlans.length ? <div className="mt-12 grid gap-6 md:grid-cols-3">{confirmedPricingPlans.map((plan) => <article key={plan.id} className="rounded-3xl border border-white/10 bg-dark-800 p-7"><h2 className="text-2xl font-bold text-white">{plan.name}</h2><p className="mt-3 text-slate-300">{plan.description}</p></article>)}</div> : <section className="mt-12 rounded-3xl border border-white/10 bg-dark-800 p-8"><h2 className="text-2xl font-bold text-white">Public plans are not available yet</h2><p className="mt-3 max-w-2xl text-slate-300">Contact the Ashur team for current commercial information. No checkout or card collection is implemented.</p><a href="mailto:ashurplatform95@gmail.com?subject=Ashur%20ERP%20Pricing" className="auth-submit mt-6 max-w-xs">Contact sales</a></section>}
