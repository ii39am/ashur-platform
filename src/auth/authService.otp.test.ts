...
describe('Supabase onboarding calls', () => {
  ...
  it('verifies a confirm-signup token using the signup OTP type', async () => { api.verifyOtp.mockResolvedValue({ data: { user: { id: 'user' }, session: {} }, error: null }); await authService.verifySignupOtp(' USER@example.com ', '123456'); expect(api.verifyOtp).toHaveBeenCalledWith({ email: 'user@example.com', token: '123456', type: 'signup' }); });
